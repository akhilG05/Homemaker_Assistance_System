...
def index(request):
	...
	profile = Contact.objects.filter(user=request.user)
	return render(request, 'mobile/index.html', {'profile':profile})

@login_required
def contact_list(request):
	contacts = Contact.objects.filter(Q(user=request.user) & (~Q(slug=str(request.user)+'RPiDefaultUser')) & Q(unknown=False))
	return render(request, 'mobile/contacts.html', {'contacts':contacts})

@login_required
def custom_contact_list(request,slug):
	contacts = Contact.objects.filter(Q(user=request.user)&Q(original_name=slug)&(~Q(slug=str(request.user)+'RPiDefaultUser')) & Q(unknown=False))
	return render(request, 'mobile/contacts.html', {'contacts':contacts})
